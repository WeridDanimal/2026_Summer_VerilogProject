...
I think I need to be sure that the array storing the receiver data gets synthesized as BRAM
    I thiiiink BRAM requires synchronous reads? 

*/


//Note, maximum # of bytes set to 3 because 24 bits are needed to accurately capture RGB info
//It is also assumed that Laptop is responsible for managing and monitoring how big the incoming data is
module largeReceiver #(parameter maxBytes=3, parameter maxRows=75, parameter maxCols=75)
...
output [1:0] eightsCount,
output reg write_en,
output reg [maxSize-1:0] write_data  ); //debug only

//if the data exceeds the standard UART message length, then you need something to track where it should be held


localparam maxSize=maxBytes*8;
localparam byteBits=$clog2(maxBytes);

localparam rowBits=$clog2(maxRows);
localparam colBits=$clog2(maxCols);
            ...
            case(state)
               ...
               begin
                    ...
                    begin
                        ...
                        if(x==inputRows-1)
                        begin
                            //$display("Received all data. RETURNING TO IDLE");
                            x<=0;
                            y<=0;
                            state<=IDLE;
                        end
